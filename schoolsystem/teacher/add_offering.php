...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['schoolLevel'])) {
        ...
        try {
            $stmt = $pdo->prepare("SELECT SubjectID, SubjectName FROM subjects WHERE SchoolLevel = :schoolLevel AND IsActive = 1");
        ...
        // This part handles the offering submission
        $term = $_POST['term'];
        $schoolYear = $_POST['schoolYear'];
        $selectedSubjects = $_POST['subjects'] ?? [];
        $schedules = $_POST['schedules'] ?? [];

        // Validation and processing logic goes here
        // For each selected subject, you should have a corresponding schedule detail
        foreach ($selectedSubjects as $index => $subjectID) {
            if (!empty($schedules[$subjectID])) {
                $scheduleDetail = $schedules[$subjectID];
                // Insert offering with individual schedule details for each subject
                // Prepare SQL and execute for each subject
            }
        }
        // Redirect or report success/failure
    }
}
...
<body>
    ...
    <div class="teacher-content">
        ...
        <?php if (!empty($subjects) && $_SERVER["REQUEST_METHOD"] == "POST"): ?>
